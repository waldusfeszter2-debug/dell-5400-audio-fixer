...
# ─────────────────────────────────────────────────────────────────────────────
# UPRAWNIENIA ADMINA – sprawdź PRZED wszystkim innym
# ─────────────────────────────────────────────────────────────────────────────
def is_admin():
    try:
        import ctypes
...
# Musi być PO sprawdzeniu admina – inaczej stary proces (bez UAC) trzyma mutex
# i nowy (z UAC) od razu wychodzi.
# ─────────────────────────────────────────────────────────────────────────────
import ctypes as _ctypes_mutex

_MUTEX_NAME = "Global\\AudioFixer_SingleInstance_v2"
...
# ─────────────────────────────────────────────────────────────────────────────
# AUTO-INSTALACJA ZALEŻNOŚCI (batch – jeden pip call tylko gdy coś brakuje)
# ─────────────────────────────────────────────────────────────────────────────
def _ensure_deps():
...
def _force_headphones_pycaw() -> bool:
    try:
        ...
        collection = enumerator.EnumAudioEndpoints(EDataFlow.eRender.value, 0x1)
...
if __name__ == "__main__":
    try:
        ...
        # ── Tray: zarządzanie widocznością ───────────────────────────────────
        # Flaga _is_withdrawn odróżnia programowe withdraw() od minimize przez użytkownika.
        # Bez niej <Unmap> odpala się też gdy kod robi withdraw() i tworzy pętlę.
        _is_withdrawn = False

        def show_window():
            global _is_withdrawn
            _is_withdrawn = False
        ...
        def on_close():
            """X w oknie → schowaj do traya."""
            hide_window()

        def on_unmap(event):
            """<Unmap> = minimize lub withdraw. Reaguj tylko na minimize."""
            if event.widget is root and not _is_withdrawn:
                # Sprawdź po chwili – stan 'iconic' jest ustawiany z małym opóźnieniem
                root.after(50, _check_minimize)

        def _check_minimize():
            if root.state() == "iconic" and not _is_withdrawn:
                hide_window()
